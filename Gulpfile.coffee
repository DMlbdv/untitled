...
config.load 'client'
config.load 'livereload'
config.load 'requirejs'
config.load 'runtime'
config.load 'server'
...
gulp.task 'svg', ->
  gulp.srcgulp.src("#{ config.client.src.svg }/**")
    .pipe(gulp.dest config.client.build.assets)
# ------------------------------------------------------------------------------
# Compile assets
# ------------------------------------------------------------------------------
...
gulp.task 'jsons', ->
  gulp.src("#{ config.client.src.views }/**/*.json")
    .pipe(gulp.dest config.client.build.assets)
    .pipe($.size())
...
gulp.task 'build', (cb) ->
  sequence = [
    'clean'
    ['vendor-js', 'vendor-css', 'vendor-etc', 'scripts', 'jade','jsons', 'images', 'public', 'stylus', 'index']
    cb
  ]
...
gulp.task 'watch', (cb) ->
  ...
  gulp.watch "#{ config.client.src.views }/**/*.json", ['jsons']
